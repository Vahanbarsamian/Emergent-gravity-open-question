...
avec (R_{\mu\nu}) le tenseur de Ricci et (R) le scalaire de courbure.
...
Cette distinction est importante pour la suite : **(G_{\mu\nu}) n'est pas le tenseur de courbure complet**, mais le tenseur géométrique qui intervient directement dans les équations d'Einstein.
...
comme une énergie physique que l'on pourrait simplement augmenter ou diminuer expérimentalement.
...
\left[
\langle
\hat{\Phi}_i\hat{\Phi}_j
\rangle,
\langle
\hat{\Phi}_i\hat{\Phi}_j\hat{\Phi}*k
\rangle,
\ldots
\right]
\quad\longrightarrow\quad
...
\xrightarrow{\text{limite semi-classique / macroscopique}}
